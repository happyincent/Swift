...
//        })
        ...
        // by ddl
        if player.rate == 0.0 {
            player.view.hideAllToasts()
            player.view.makeToast("x 1.0")
            return
        }
        
        let rateList :[Float] = [0.5, 0.75, 1.0, 1.25, 1.5]
        let newRate = rateList[ (rateList.index(of: player.rate)! + 1) % rateList.count ]
        
        print("Rate:", player.rate, "->", newRate)
        player.rate = newRate
        
        player.view.hideAllToasts()
        player.view.makeToast("x \(newRate)")
        
        let currentTime = player.currentTime ?? 0
        let duration = player.duration ?? 0
        
        xAPISender_Player().send(
            URL: player.contentURL?.absoluteString ?? "error",
            oldState: "changeRate",
            currentTime: Int( currentTime.isNaN ? 0 : currentTime ),
...
//            }
//        }
        self.audioSubtitleCCButtonWidthConstraint.constant = 50 // change rate (by ddl)
